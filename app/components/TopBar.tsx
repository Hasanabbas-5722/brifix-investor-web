...
export default function TopBar() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        const detail =
          data?.status_detail ||
          (isOpen ? 'Market is Open · Closes at 03:30 PM IST' : data?.schedule?.next_session_label || 'Market is Closed · Opens at 09:15 AM IST');
        setMarketStatus({
          isOpen,
  ...
  const initial = (displayName || 'B').replace('@', '')[0]?.toUpperCase() || 'B';
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginLeft: 'auto' }}>
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            onMouseEnter={(e) => {
              if (!notifOpen) e.currentTarget.style.borderColor = 'var(--border-strong)';
            }}
            onMouseLeave={(e) => {
              if (!notifOpen) e.currentTarget.style.borderColor = 'var(--border)';
            }}
        ...
        >
          ...
          >
            <span style={{ color: '#fff', fontSize: 12, fontWeight: 800 }}>{initial}</span>
          </div>
          <div className="hidden md:block" style={{ textAlign: 'left', lineHeight: 1 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-1)', marginBottom: 2 }}>{displayName}</p>
            <p
              style={{
                fontSize: 10,
                color: isPremium ? 'var(--amber)' : isPro ? 'var(--accent-light)' : 'var(--text-3)',
                fontWeight: 600,
